...
Reference: @src/shared/room/util/roomValidationUtil.ts , @src/shared/object/types/objectTypeConfig/doorObjectTypeConfig.ts , @src/shared/object/types/objectTypeConfig/labelObjectTypeConfig.ts , @src/server/user/util/userIdentificationUtil.ts
...
- Putting up, moving, editing and removing labels (signs on a wall; see [label_text.md](../graphics/label_text.md)), under the same rule as doors.
...
## Enforcement
- `RoomValidationUtil` answers both "is this user an admin" and "may this user edit this room's doors". Door and label operations run that check on the client and the server.
...
## UI
There is no separate admin mode. Admins see extra tools: adding a door or a label to a selected wall, selecting doors and labels in edit mode, and room settings in hubs. Locally, a dev admin is available through `?devuser=` or `?sandboxadmin=` (see [local_dev.md](../devOps/local_dev.md)).
