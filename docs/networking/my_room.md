...
1. The client shows `VisitMyRoomForm`, prompting the user to confirm. When confirmed, the client checks `user.ownedRoomID`:
    - If the user does not yet own a room (`ownedRoomID` is empty):
        ...
        4. The client updates `user.ownedRoomID` with the returned `roomID`, then emits a `roomChangeRequestParams` via `SocketsClient.tryEmitRoomChangeRequest`.
    - If the user already owns a room (`ownedRoomID` is non-empty):
        1. The client directly emits a `roomChangeRequestParams` via `SocketsClient.tryEmitRoomChangeRequest`, using the existing `ownedRoomID`.
2. The server receives the `roomChangeRequestParams` and calls `RoomManager.changeUserRoom()`, moving the user into their room.
...
3. The server calls `DBUserRoomStateUtil.setUserRole()` to persist the Editor role for the target user, then calls `syncUserRoleInMemory()` to update the in-memory role map and broadcast a `userRoleUpdateParams` signal to all clients currently in the room.
...
3. The server calls `syncUserRoleInMemory()` to update the in-memory role map and broadcast a `userRoleUpdateParams` signal to all clients currently in the room.
