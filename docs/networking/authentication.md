...
3. The server identifies the user:
    ...
    A token names an account, and the browser's cookie is the only copy of it — so issuing a different one is how a session is lost, not merely interrupted. Two distinctions keep that from happening by accident. First, a lookup that *fails* is not read as an account that is *absent*: the failure says nothing about whether the account exists, so the request is refused and the cookie left alone, rather than answered with a guest whose token would displace the one the browser was holding. Second, only routes that serve anonymous visitors mint guests at all. A route reserved for members or admins is never somebody's first contact with the site, so an account created there is one the permission check discards in the same breath — after its token has already been handed to the browser.
