...
2. The server receives the `SetObjectTransformSignal` and passes it over to `ServerObjectManager.updateObjectTransform`.
3. `ServerObjectManager.updateObjectTransform` checks the object's type configuration to determine whether it has a `dynamicCollider` component:
