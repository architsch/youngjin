...
Every `PhysicsRoom` holds a fixed `globalColliders` set that bounds the playable volume (`x ∈ [0, NUM_VOXEL_COLS]`, `z ∈ [0, NUM_VOXEL_ROWS]`, `y ∈ [0, MAX_ROOM_Y]`):
- **Six perimeter colliders** — floor, ceiling, and four side walls. Each is a 100 × 100 × 100 cube (built by `makeCubeCollider`) placed flush just outside one face of the room, so its 50-unit thickness extends away from the playable volume. They share one hard-collision config (`applyHardCollisionToOthers: true`, no soft push).
  - **Floor**: top face at y = 0. **Ceiling**: bottom face at y = MAX_ROOM_Y (= 4).
  - **Walls `lowerX` / `upperX`**: inner faces at x = 0 and x = NUM_VOXEL_COLS. **Walls `lowerZ` / `upperZ`**: inner faces at z = 0 and z = NUM_VOXEL_ROWS.
- **Entrance collider** — an invisible plug over the doorway opening in the upper-Z wall. See [room_entrance.md](room_entrance.md#the-entrance-collider-plugging-the-opening).
...
- Queries convert AABB bounds to voxel grid coordinates via `floor()`, clamped to the valid grid range (`[0, NUM_VOXEL_COLS-1]` / `[0, NUM_VOXEL_ROWS-1]`).
- The room's `globalColliders` (boundaries + entrance) are always tested; in addition, only voxels within the AABB's XZ footprint are checked, giving O(1) collision checks for any given movement.
