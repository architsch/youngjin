...
- A codec built around **parameters** (a player's slots, a door's colors, a canvas's or lamp's frame and margin) stores those and rebuilds its parts from them, so it can offer presets, seeded defaults and a bounded set of variants. `DefaultCompositionCodec` instead **spells out arbitrary parts**, and is what authored data is pre-encoded through.
...
- An object may draw its own content over a composed part, as a door's label sits on its plate. A canvas is one moulded board, drawn a margin inside its footprint: the band is the frame, and the picture covers the surface inside it, placed from the decoded parameters so it follows any edit. A canvas without a frame composes no parts, and its picture covers what the board would. The board follows whatever the canvas has been resized to while the band keeps its width, because the wood material measures the band in world units; the margin and the picture's inset are absolute for the same reason. Where the margin would leave no picture, the margin gives way rather than the band (`MarginCompositionConstants`).
- A lamp is the same board with a composed glow in place of a picture, and the same margin. The glow is offset in depth beyond the wood, as a picture is, so the board never shows through it at an angle.
