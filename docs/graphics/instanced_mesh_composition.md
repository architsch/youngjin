...
- An object may draw its own content over a composed part, as a door's label sits on its plate. A canvas is one moulded board: the band is the frame, and the picture covers the surface inside it, inset by the decoded board's band so it follows any edit to it. A canvas without a frame composes no parts, and its picture covers the whole footprint. The board spans whatever the canvas has been resized to while the band keeps its width, because the wood material measures the band in world units; the picture's inset is absolute for the same reason.
