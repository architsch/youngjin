...
- Builders register themselves in `InstancedMeshCompositionBuilderMap` only when `@src/shared/graphics/mesh/composition/instancedMeshCompositionBuilderMapDependencies.ts` is imported. The client and SSG import it at their entry points, but the game server doesn't, so shared code that decodes compositions on the server (e.g. `ObjectGroupVersionMigration`) must import it itself. Integration tests don't register builders globally, so a missing import fails the tests.
