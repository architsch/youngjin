...
- **`OrbitCameraPose`**:
  ...
  - The framing distance scales with the target's size, and a selection can require a minimum distance (a block or wall object is framed with its surroundings). Zoom is a multiple of the framing distance within a range, published logarithmically through `orbitCameraZoomObservable` for the zoom slider.
...
- The head light follows the eased camera. Its range grows with distance and its intensity compensates for falloff, so the target keeps its brightness.
