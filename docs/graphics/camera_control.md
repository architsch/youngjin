...
The user's own body — and the speech bubble hanging over it — is in view for as long as the camera is pulled back from it, which is to say throughout edit mode, whatever is currently selected in it. The character is one of the things the user came into that mode to work on, and it stays on show even while he works on something else. The first-person view is the one that hides it, so that it never clips the camera; `PlayerGameObject` watches the camera mode alone to decide this. Wherever the body does get between the camera and what is being framed, it is taken out of the way like anything else that does (see [Clearing the Line of Sight](#clearing-the-line-of-sight-orbitocclusionhider)). (Other players' bodies are governed by proximity instead: a player who comes too close to the user is temporarily hidden so it does not clip through the camera.)
...
Which way the target is currently viewed from is published in world terms (`orbitCameraAnglesObservable`), on the same footing as the zoom below: the drag reads it and writes it back, so setting it is a way of pointing the camera somewhere. That is how a scripted tutorial step arranges the view it wants the user to begin from — and, the angles being known, how a later step can tell that the user has since moved the camera himself. World terms rather than terms relative to the player, so that the same angles frame the same thing however the user happened to be turned.
