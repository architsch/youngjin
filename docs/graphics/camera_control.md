...
- **Everything else.** Canvases, doors, and players are found by raycasting the remaining meshes — few enough to raycast cheaply — along a grid of samples spread over the target's silhouette, dense enough that an object cannot sit unnoticed between two samples. An object a ray strikes is hidden whole rather than at the point struck: a player is drawn as the set of parts he is composed of, and taking away only the part a sample happened to land on would leave the rest of the body standing in front of the target.
