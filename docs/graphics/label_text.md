...
Reference: @src/client/object/components/labelText.ts , @src/client/graphics/types/textureAtlasAllocator.ts , @src/client/object/util/labelTextLayoutUtil.ts , @src/shared/object/util/labelTextUtil.ts , @src/shared/math/util/htmlUtil.ts , @src/shared/object/types/objectTypeConfig/labelObjectTypeConfig.ts
...
- A line break in the text starts a new line; within a line, wrapping is automatic. A line is as tall as its largest lettering.
- **Markup**: a few HTML tags style parts of the text — `<b>`, `<i>`, `<u>`, `<s>`, and `<font>` with `size` (read as browsers read it) and `face` (`serif`, `sans-serif`, `monospace`). `HTMLUtil` reads only the tags `LabelTextUtil` allows; any other tag stays in the text as typed, and nothing is handed to the DOM. No part can take its own color, since the atlas holds only coverage.
...
- A door's label, as read (no markup, whitespace collapsed), is also the name other doors look it up by (see [room_entrance.md](../geometry/room_entrance.md)).
