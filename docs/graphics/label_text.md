...
Reference: @src/client/object/components/labelText.ts , @src/client/graphics/types/textureAtlasAllocator.ts , @src/client/object/util/labelTextLayoutUtil.ts , @src/shared/object/util/labelTextUtil.ts , @src/shared/object/types/objectTypeConfig/labelObjectTypeConfig.ts
...
- **Fixed size**, in atlas pixels: a word wider than a line breaks between characters, and lines past the bottom are cut off (the text is top-aligned once it overflows).
- Wrapping is always automatic, since the text is typed on one line.
