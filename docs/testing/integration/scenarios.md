...
| `object-transform-migration.test.ts` | transform ranges and their migration, the scale a stored object comes back at, older lamps keeping their size, framed lamps losing their look and shrinking in place to the largest size, lamp colors moving to the nearest one the trimmed "Light" palette offers, and label font sizes moving to the nearest size on offer |
...
| `composition.test.ts` | player, door and indexed mesh composition codecs; a door's finishes on offer, its default and which finishes it accepts |
| `canvas-frame.test.ts` | the Default codec's wood parts, the framed panel codec (round trip, no frame, untrusted input, resizing), a canvas's looks (frameless first), defaults and permissions, the bitmap-frame migration, the migration of canvas, label and door finishes to their nearest look, the per-type pre-encoded table (every indexed type present), the thumbnail atlas layout |
| `label.test.ts` | label permissions (the doors' rule, the metadata whitelist, its own looks only, the cap), the lettering settings (round trip, untrusted input, nearest size, clamping, defaults), the name a label reads as (whitespace collapsed, markup kept as typed), text length in characters, a label's looks (frameless first, then plaques) and default, and a room at every cap holding the longest text fitting the encoding buffer |
| `label-atlas.test.ts` | atlas allocation (no shared or out-of-bounds cells under any sequence, reuse, a room's worth of labels always fitting once packed largest first), lettering layout (auto size fits, line breaks included, even lines, blank lines, fixed size breaking long words and leaving overflow below), and the bundled label font (read from its own tables as fontTools reads them, widths that add up, missing characters drawn as U+FFFD) |
...
| `room-prefs.test.ts` | room prefs encoding, defaults, head-lamp power, light palettes and carrying light colors over from the six-saturation palette, sky color |
