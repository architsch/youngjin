...
The server generates every Hub/Regular room from a seed drawn at creation time — its layout, its contents and its texture pack alike — which makes a real room a poor fixture: a scenario that builds and edits blocks at chosen coordinates needs to know what is already there, and needs the same answer on every run. So `createTestRoom()` gives scenarios the bare shell instead — one open floor per storey inside the boundary wall, divided by the storey floor slab, with the room's own door hung on the boundary at its entrance cell, in a fixed texture pack and with nothing else in it — and both `mockDB.seedRoom()` and `selectionHarness.createRoom()` go through it. Every storey a real room can have is therefore there to build on, while none of them has anything standing in it. Single-player rooms keep their real template, since that template *is* what those scenarios are about.
