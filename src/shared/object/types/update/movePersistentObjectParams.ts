...
// We encode dx and dy as: rawByte = floor(2 * offset) + 128
...
{
    objectId: string;
    dx: number; // horizontal offset (x-axis)
    dy: number; // vertical offset (y-axis)

    constructor(objectId: string, dx: number, dy: number)
    {
    ...
    encode(bufferState: BufferState)
    {
        ...
        new EncodableRawByteNumber(Math.floor(4 * this.dy) + 128).encode(bufferState);
    }
    ...
    static decode(bufferState: BufferState): EncodableData
    {
        ...
        const dy = 0.25 * (dyRaw - 128);
        return new MovePersistentObjectParams(objectId, dx, dy);
    }
}
