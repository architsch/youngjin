...
export function getVoxelsInBox(physicsRoom: PhysicsRoom, box: AABB2): PhysicsVoxel[]
{
    ...
    const row2 = Math.min(NUM_GRID_ROWS-1, Math.floor(box.y + box.halfSizeY));
    const col2 = Math.min(NUM_GRID_COLS-1, Math.floor(box.x + box.halfSizeX));
    for (let row = row1; row <= row2; ++row)
    {
        for (let col = col1; col <= col2; ++col)
        {
            const voxel = physicsRoom.voxels[row * NUM_GRID_COLS + col];
            if (voxel != undefined)
