...
// around its border, out of which whole objects are composed by laying smaller panels over larger
// ones (see DoorCompositionCodec). Nothing here comes from an image.
//
// Three things make that work.
//
// The moulding is measured in world units rather than in the quad's own coordinates, so a band stays
// the same width whether it frames a door slab or a doorknob — a moulding that stretched with its
// panel would read as a different profile on every part. The instance's world extent is recovered
// from its transform for this, the same way the tin material recovers its scale.
//
// The timber's figure is likewise sampled in world space, along the axes of the quad it is drawn on
// rather than within the quad. Nothing about it refers to the part's size, so the grain is as fine on
// a knob as on the slab behind it; and because neighbouring parts of one object sample the same
// field, the figure runs on through all of them, as if the object had been cut out of a single board.
//
// Finally, what makes a moulding visible is its relief and not its color. The band's profile is
// shaded as if a raking light fell across the object — one flank of a bead lit, the other in shadow,
// and a sunk region holding its own shadow throughout — and that shading is what tells a carved panel
// from the field around it. (The scene's only lamp rides the camera, so tilting the shading normal
// alone would darken both flanks of a bead symmetrically, which reads as a line drawn on a flat
// surface rather than as something cut into one.) The shading normal is tilted along the profile as
// well, but only lightly, and for the sake of the specular: it is what gives a moulding a highlight
// that travels along it as the viewer moves.
//
// The per-instance inputs are the surface color (the instance color), the moulding's own color, and
// the band's width and whether its profile stands proud of the surface or is sunk into it.
...
const NORMAL_FRAGMENT_GLSL = `
    ...
    // at the peak of the specular lobe at once (see the tin shader for why).
...
export default function installInstancedWoodShader(shader: THREE.WebGLProgramParametersWithUniforms)
{
    ...
    // Injected after the normal is resolved, for the same reason the tin shader injects there: this
    // is the last point ahead of <lights_phong_fragment>, which is what finally reads the normal
    // being modified here.
    shader.fragmentShader = shader.fragmentShader.replace(
...
// What the material itself must be set to for the shader above to read as waxed joinery. Kept beside
// the shader, since the finish and the code that shapes it are one description of one surface.
