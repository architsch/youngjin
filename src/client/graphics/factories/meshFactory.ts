...
const MeshFactory =
{
    load: async (geometryId: string, textureId: string): Promise<THREE.Mesh> =>
    {
        const meshId = `${geometryId}-${textureId}`;
        const loadedMesh = loadedMeshes[meshId];
        if (loadedMesh != undefined)
            return loadedMesh;

        const geometry = await GeometryFactory.load(geometryId);
        const material = await MaterialFactory.load(getTextureURL(textureId));
