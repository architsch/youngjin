...
const UserAuthUtil =
{
    authenticateUserFromReq: async (req: Request, res: Response,
        passCondition: (user: User) => Boolean, next: () => void): Promise<boolean> =>
    {
        ...
        {
            ServerLogUtil.logRaw("Authentication Failed", "high", "pink");
...
function getUserFromReq(req: Request): User | undefined
{
    ...
    if (!user)
    {
        console.log(`############ User not found (${token as string}) --- (${JSON.stringify(Object.entries(req.cookies))})`);
