...
export default class LibraryPageBuilder
{
    ...
    addPostLinks(builder: TextFileBuilder, title: string, entries: PostEntry[]): void
    {
        ...
        for (let i = entries.length - 1; i >= 0; --i)
        {
            ...
            builder.addLine(`<a class="postEntryButton" href="${process.env.URL_STATIC}/${entry.dirName}/list.html">${entry.title}</a>`);
        }
    }
    ...
    async build(): Promise<void>
    {
        ...
        builder.addLine(await EJSUtil.createStaticHTMLFromEJS("partial/common/header.ejs", {
            url: `${process.env.URL_STATIC}/library.html`,
            pageName: "library",
            pagePathList: [
                {title: UIConfig.displayText.pageName["index"], url: process.env.URL_STATIC},
                {title: UIConfig.displayText.pageName["library"], url: undefined},
            ],
