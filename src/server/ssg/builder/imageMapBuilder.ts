...
export default class ImageMapBuilder
{
    ...
    // Writes a thumbnail of each image beside it: the image scaled down to fit inside a square of the
    // thumbnail size, keeping its aspect ratio, and left as it is if it already fits.
    ...
    private async writeMapFile(subfolderInfoByName: {[subfolderName: string]: ImageMapSubfolderInfo}): Promise<void>
    {
        ...
        // NOTE: the import paths below are relative to the generated file (MAPS_ROOT_PATH), not to this builder.
        // Do not let an IDE "update imports on move" refactor rewrite them.
