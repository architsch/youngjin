---
name: release-train
description: Run the full release pipeline for a batch of changes — audit the project's own skills and dev tooling, write a dev-log post, commit and push, watch the GitHub Actions deployment to staging and fix what fails, run VPS maintenance, then playtest the deployed staging server. Use when asked to ship, release, run the release train, take a feature all the way to staging, or run the whole end-to-end workflow.
---
...
| # | Phase | Runs as | Skill |
|---|---|---|---|
| 1 | The project's own skills and dev tooling are audited | subagent | `skill-upkeep` |
| 2 | A dev-log post is written | subagent | `devlog-post` |
...
- **Phase 1 (`skill-upkeep`) owns** `.claude/skills/`, `.claude/writing-style.md`, `.claude/rules/`,
  the dev tooling under `dev/scripts/` and the guideline pages under `docs/devOps/` and
  `docs/testing/`.
- **Phase 2 (`devlog-post`) owns** `public/devlog-<year>/source.txt`.

Neither may touch `src/`, `tests/`, the rest of `docs/`, `CLAUDE.md` or `README.md`. If either
concludes a change there is needed, it reports that as a finding and does not make it — a skills
audit that quietly edits application code is an audit whose diff nobody will read carefully.
...
Phase 2 writes text and nothing else: no dev server, no screenshots, no site generation. The pages
are generated by phase 3's build, and the screenshots are the user's to add at checkpoint B. One
check when it reports back, because a post that never landed in the file looks exactly like one that
did, and it is a grep rather than a read:

```bash
DEVLOG=$(node -e "console.log(require('./dev/scripts/devlog/devlogDir').resolveDevlogDir().source)")
grep -c "<the post's title>" "$DEVLOG"
```
...
- The post's title and the page number it will become.
- The **full post text as the user will paste it**, in a copyable block, with its character count.
- A reminder that the **screenshots are theirs to add** — the JPEGs into `public/devlog-<year>/` and
  their `<name>` lines into the block — and that phase 3's build is what turns the block into a
  page, so anything added after the commit misses this release.
...
1. **Fold the advice into the skill files**, on the main thread, before regenerating anything. Put
   ...
   - `devlog-post/reference/post-format.md` for the source format, `reference/hashtags.md` for the
     tag bank.
...
2. **Delegate phase 2 again** with the same feature named, briefing it on the user's note and on
   ...
   appended after — otherwise the site grows a duplicate post. The page number stays the same, and
   any image lines the user has already added stay where they are.
...
The user may also decide to drop the post here. Then remove its block from `source.txt`, ask what to
do with any screenshots they had already added for it, and go on to phase 3 without a post.
